...
| `npm run program:redate -- --program <template>` | Move a program's dates to what its file says, in place: each step whose date differs gets the file's date on the program's row and on every participation's copy that still carried the old one (a copy moved by hand is left). Dry by default; `--apply` does it; `--cloud` for the hosted project; `--org <slug>` for one school. Nothing a student wrote is touched. |
...
| `test:redate` | The interviews reshaped and redated without a reset: the deliverable keeps its id and the old field ids, three boxes required and seven on demand with a live tally, no showcase block; the two dates resolve as asked; `program:redate` is dry by default, writes only a date and its note, and leaves a participation's copy that was moved by hand. |
