...
| `npm run demo:refresh` | Correct the demonstration tenant's two invented records in place (dates, the four drawn pictures, the video still) and rebuild the records index; no reset. `-- --cloud --allow-remote=<ref>` for the hosted project. Refuses any organization without `demo: true`. |
| `npm run onepager -- --base <url>` | A printable one-page handout for the showcase: the journal's name, the address, the top of the showcase and the featured record through its citation, photographed from the live site into `local-data/onepager/<slug>/handout.pdf`. `--record`, `--title`, `--sub`, `--url` override the defaults. |
| `npm run program:redate -- --program <template>` | Move a program's calendar to what its file says, in place (dev-152, dev-160): a step whose date differs gets the file's date on the program's row and on every participation's copy that still carried the old one (a copy moved by hand is left); a step whose shape differs — required, what it waits on, name, phase, order, deliverable — gets that on the row and every copy; a step in the file with no row is added with a copy for every participation, from the same row builder the seed uses (`scripts/milestone-rows.mjs`); a row with no step in the file is reported and left. One `audit_log` row per change (`milestone.redated`, `milestone.added`), which `report:day` lists. Dry by default; `--apply` does it; `--cloud` for the hosted project; `--org <slug>` for one school. Nothing is deleted and nothing a student wrote is touched. |
...
| `npm run pilot:password -- <email>` | Set a password on one staff or advisor account so it can sign in without Google: generated, or `--set '<phrase>'`. `--cloud` for the pilot. Students are refused — unless the teacher asks for the class: `--for-class` sets one on a named student (printed once), and `--roster local-data/pilot-irpd.yaml --for-class --out local-data/pilot-passwords.csv` gives every account on the roster its own distinct phrase, written to that file (gitignored, mode 600) and printed nowhere, with an audit line on each; each row carries the name, the role in the class (Student, Elder, Teacher), the group, the project title and the address, so an anonymised roster still says who is who. Google sign-in keeps working beside it. |
| `npm run pilot:anonymize` | The roster with no people in it: every student becomes `S00`, `S01`, …, every Elder `E00`, `E01`, … (`--me <name>` is E00; default Rohan Agarwal), every teacher `T00`, `T01`, …, drawn at random; `key`, `name`, `email` (`<id>@scipath.org`), a group's `elders`, a project's `authors` and a score's `student` and `by` change together, both domain fields become `scipath.org`, and nothing else in the file changes. Refuses to write if any real key, name or address survives in the output. `local-data/pilot-irpd.yaml` → `local-data/pilot-irpd.anon.yaml`, with the drawing beside it as `pilot-irpd.anon.idmap.csv` (mode 600), the only way back. `--in`, `--out`, `--map`, `--domain`. Touches no database. |
| `npm run report:day -- --cloud` | The day in numbers, with nobody in it: sign-ins, who saved and how much (saves, documents, words written), submissions, questions asked and answered with the median wait, Elder scores, where every step of the class stands, transport incidents, wrong-password counts, and (dev-155) the week's actives, the feedback loop (hours to first feedback, revisions within 48 h of a comment), saves outside class hours, interview counts, (dev-156) the survey — shown, answered, dismissed and the response rate, per question — and (dev-157) quiet projects (nothing in seven days), the student's response to a staff line (median hours to the next save), reminders sent, and the teacher's Friday minutes-saved series, and (dev-161) of the submitted documents how many carry a photo or PDF, how many typed work, per deliverable — today (Pacific) beside since-the-start — written to `local-data/reports/<date>.json` and `.md`. People appear as their identifiers (S07, E00) or their role; no address and no typed text leaves the database, and the file is checked for both before it is written. `--date YYYY-MM-DD` for another day, `--org <slug>`, `--out <dir>`. Reads with the secret key; writes only files. |
| `npm run who:active -- --cloud [--minutes 15]` | Who has written anything (a field, a line, a note, a score) in the last N minutes, as identifiers and roles, newest first — the thing to look at before a restart. Reads only. |
| `npm run today -- --cloud` | The day so far, on the terminal, nothing written: who is writing right now (last 15 minutes), the hour's saves and submissions, open questions, then today's sign-ins, writing, questions, scores and incidents, the busiest people and where the steps due so far stand. The same numbers as `report:day`. |
...
| `npm run reset:cloud` | **Destroys the hosted project's data**: `node scripts/reset-cloud.mjs --yes --project=<ref>`, and it will not run without both. `--keep-storage` leaves R2 alone. `--from "<step>"` (for example `--from "Journal back catalogue"`) resumes the seeds from a named step after a dropped connection stopped one, leaving the schema, the accounts and the files as they are — every seed is safe to run twice, and the failure message prints the exact command. Writes to R2 are retried three times and name the key and the cause when they still fail. Not for a project holding real work (decision 71). |
...
| `test:demo` | The demonstration records and the showcase front: the invented mark is per record and derived from the tenant's file, a real school never shows it, the featured rule never leads with an invented record, the seed dates fixtures once, `demo:refresh` refuses a real school before it reads anything, and the pages open with search, featured, year and subject before the list. |
| `test:idle` | An hour away signs a person out: the stamp is set by their requests and not the page's polling, expires past the hour and never before, the sign-in page says why, the device's clock is the same hour; indented paragraphs render one level per four spaces and fenced code stays code; the references field hangs; the profile's password form posts to the one handler with the address from the session. |
| `test:redate` | The program changed under the class without a reset: the interviews (three boxes required, seven on demand, a live tally, no showcase), the dates as asked (9/21, 10/8, 9/14, WBS 9/15 and its three promises 10/15, 11/15, 12/15), no student step waiting on an Elder's; and `program:redate` run for real over a fake database (`tests/fake-db.mjs`): dry writes nothing, apply moves the date, lifts the gate and the order, leaves the copy moved by hand, adds the three steps with a copy each, records every change, and a second run is silent. |
| `test:survey` | The survey (dev-156): the rules — the Workbench sampled one load in four, no question before three days and a first save, six showings a person ever, seven days between them (doubled after three dismissals in a row), each question once, a question dismissed twice retired; 0002 adds a table no session writes, the trail and the recorder; one card on both surfaces posting to one route; the report counts shown, answered and dismissed and never a line, and still carries the deeper measures (the week's actives, the feedback loop, after-hours saves, interview counts). (dev-157) The teacher's weekly question: from Friday, once a week, on every load until answered or put off, in the school's time, outside the students' rules; a number answer kept as given; `activity_days` rolled up from every save by a trigger, backfilled once, read by whoever may see the project and written by nobody. |
...
| `test:context` | The context cookie: `my_context()`'s answer kept for a minute, signed with a key derived from the project's secret, per person; edited, extended, borrowed or oversized cookies refused; cleared by every write except the autosave and the pulse, and by the auth routes. |
| `test:change` | A password changed with the current one (/auth/change/): one sentence for every failure, guesses counted before the credential on both doors, the count held with the secret key only (`password_attempts`), and a school with `sign_in: password` showing no Google, linking the change page, and refusing a Google session. |
| `test:editor` | The rich text editor, driven in a real Chrome: its script compiled as it is, mounted on its own markup, and worked by keys and clicks (Heading 1, Enter, Heading 2 lands on the new line; a heading at the start of a line stays there; a list starts). Skips without a Chrome (`CHROME=/path`). |
| `test:build-id` | A page left open across a deploy: the build names itself once, every response carries it (`x-scipath-build`), the shell compares on responses it already gets and asks once when a hidden tab is looked at again, never polls, and reloads only when nothing is typed or saving. |
...
| `test:standin` | Upload it instead (dev-161): every shape carries the uploads section first (five file boxes, four on demand, image or PDF) unless it opts out; one upload completes a document whatever the typed fields say, typing as well changes nothing, a removed upload does not count; the page marks the stand-in fields, says so in the strip, mirrors the rule in the browser, shrinks a large photo before it goes up, and gives a teacher "Remove this file"; the upload route caps files per document and still refuses an SVG or an executable; the media route serves a removed file to nobody; the report counts uploaded against typed per deliverable and carries no path. |
